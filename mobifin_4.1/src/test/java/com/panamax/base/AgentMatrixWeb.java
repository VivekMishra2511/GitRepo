...
public class AgentMatrixWeb extends Common {
	 ...
	 */
	public AgentMatrixWeb(WebDriver driver) {
		this.driver = driver;
	}

	/**
	 * @author kutbuddin.compounder select value from Agent Matrix profileName
	 *         drop down
	 * @param profileName
	 * @creation date 22/11/2018
	 */
	...
	/**
	 * @author kutbuddin.compounder select value from Agent Matrix treeName drop
	 *         down
	 * @param treeName
	...
	/**
	 * @author kutbuddin.compounder select value from Agent Matrix treeLevel
	 *         drop down
	 * @param treeLevel
	 * @creation date 21/11/2018
	 */
	...
	/**
	 * @author kutbuddin.compounder send text in Agent Matrix productCode field
	 * @param productCode
	 * @creation date 22/11/2018
	 */
	...
	/**
	 * @author kutbuddin.compounder select value from Agent-Matrix status drop
	 *         down
	 * @param status
	 * @creation date 22/11/2018
	 */
	...
	/**
	 * @author kutbuddin.compounder select value from Agent-Matrix Kyc status
	 *         search drop down
	 * @param kycStatus
	 * @creation date 22/11/2018
	 */
	...
	/**
	 * @author kutbuddin.compounder select value from Agent-Matrix productNAme
	 *         search drop down
	 * @param productNAme
	 * @creation date 22/11/2018
	 */
	...
	/**
	 * @author kutbuddin.compounder select value from Agent-Matrix serviceType
	 *         search drop down
	 * @param serviceType
	 * @creation date 22/11/2018
	 */
	...
	/**
	 * @author kutbuddin.compounder select value from Agent-Matrix operatorName
	 *         search drop down
	 * @param operatorName
	 * @creation date 22/11/2018
	 */
	...
	/**
	 * @author kutbuddin.compounder send text in from Agent-Matrix
	...
	/**
	 * @author kutbuddin.compounder to Edit Agent Matrix
	 * @param map
	 *            - excel values
	 * @param mapkeys
	 *            - excel header
	 * @creation date 22/11/2018
	 */
	public void editAgentMatrix(Map<Object, Object> map, List<Object> keys) {
		if (!map.get(getMapKeys(map).get(0)).toString().isEmpty()) {
			selectTreeName(map.get(getMapKeys(map).get(0)).toString());
			selectTreeLevel(map.get(getMapKeys(map).get(1)).toString());
		}
		selectProfileName(map.get(getMapKeys(map).get(2)).toString());
		...
		for (int i = 0; i < Services.length; i++) {
			...
			if (!Services[i].isEmpty()) {
				...
				if (EditServices[0].trim().equalsIgnoreCase("Internal Services")) {
					...
					if (!searchFilterValues[3].equals(" ")) {
						selectSearchKycStatus(searchFilterValues[3].trim());
					}
					if (!searchFilterValues[4].equals(" ")) {
				...
				for (int j = 0; j < surchargeType.length; j++) {
					...
					By drpCommissionType = By.xpath("(//select[contains(@id,'CommissionType')])[" + (j + 1) + "]");
					...
					if (verifyElement(lblLevel1, false)) {
						...
						sendTextInValue(txtlevel1Value, cashbackValue1[countVlue].trim());
					}
					if (verifyElement(lblLevel2, false)) {
						By txtlevel2Value = By.xpath("(//*[contains(@id,'Level2')])[" + (j + 1) + "]");
						sendTextInValue(txtlevel2Value, cashbackValue1[(countVlue + 1)].trim());
					}
					if (verifyElement(lblLevel3, false)) {
						By txtlevel3Value = By.xpath("(//*[contains(@id,'Level3')])[" + (j + 1) + "]");
						sendTextInValue(txtlevel3Value, cashbackValue1[(countVlue + 2)].trim());
					}
				}
				clickOnSaveBtn();
				count++;
	...
	public void clickOnInfoBtnOfInternalService(String trim) {
		clickOnElement(By.xpath(
				"(//td[contains(text(),'" + trim + "')]//preceding-sibling::td[@class='rest-table-action'])[1]"));
	}

	/**
	 * @author kutbuddin.compounder to verify Agent Matrix edited?
	 * @param map
	 *            - excel values
	 * @param keys
	 *            - excel header
	 * @creation date 22/11/2018
	 * @return true if Agent Matrix edited successfully
	 */
	public boolean verifyAgentMatrixEdited(Map<Object, Object> map, List<Object> keys) {
		String[] Services = map.get(getMapKeys(map).get(3)).toString().split("/");
		int count = 0;
		for (int i = 0; i < Services.length; i++) {
			String[] EditServices = Services[i].split(":");
			if (!Services[i].isEmpty()) {
				...
				for (int j = 0; j < surchargeType.length; j++) {
					...
					if (getUIText(SurchargeValue).contentEquals(surchargeValue[j].trim())) {
					...
					if (verifyElement(lblVerifyLevel2, false)) {
						...
						if (getUIText(txtlevel2Value).contentEquals(cashbackValue1[(countVlue + 1)].trim())) {
					...
					if (verifyElement(lblVerifyLevel3, false)) {
						...
						if (getUIText(txtlevel3Value).contentEquals(cashbackValue1[(countVlue + 2)].trim())) {
	...
	/**
	 * @author dishant.doshi to sort agent matrix
	 * @param map
	 *            - for excel data
	 * @param mapKeys
	 *            - for excel header
