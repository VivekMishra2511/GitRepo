...
/**
 * @author dishant.doshi Actions of Agent - Profile
 * @creation date 12/11/2018
 */
public class AgentProfileWeb extends Common {
	...
	By drpTreeLevel = By.id("TreeNodeLevel");
	 ...
	 * @creation date 12/11/2018
	 */
	public AgentProfileWeb(WebDriver driver) {
		this.driver = driver;
	}
	...
	/**
	 * @author dishant.doshi to select text in tree name
	 * @param treeName
	 * @creation date 12/11/2018
	 */
	public void selectTextInTree(String treeName) {
		selectFromDropdown(drpTree, treeName);
	}

	/**
	 * @author dishant.doshi to select text in tree node level
	 * @param treeNodeLevel
	 * @creation date 12/11/2018
	 */
	public void selectTextInTreeNodeLevel(String treeNodeLevel) {
		selectFromDropdown(drpTreeLevel, treeNodeLevel);
	}

	/**
	 * @author dishant.doshi to select text in parent profile
	 * @param parentProfile
	 * @creation date 12/11/2018
	 */
	public void selectTextInParentProfile(String parentProfile) {
		selectFromDropdown(drpParentProfile, parentProfile);
	}
	...
	public void clickOnAccessChannel(String accessChannelName) {
		clickOnElement(By.xpath("//span[text()='" + accessChannelName.toString() + "']"));
	}
	...
	public void addProfile(Map<Object, Object> map, List<Object> mapKeys) {
		...
		if (!verifyElement(By.xpath("(//td[text()='" + map.get(mapKeys.get(0)).toString() + "'])[1]"), false)) {
			...
			selectTextInTree(map.get(mapKeys.get(2)).toString());
			selectTextInStatus(map.get(mapKeys.get(5)).toString());
			if (verifyElement(drpTreeLevel, false)) {
				selectTextInTreeNodeLevel(map.get(mapKeys.get(3)).toString());
				commonWait();
				selectTextInParentProfile(map.get(mapKeys.get(4)).toString());
			}
			clickOnNextBtn();
			if (verifyToolTip()) {
				clickOnCancelBtn();
				...
				for (int i = 0; i < navigationInKYCDetails.size(); i++) {
					if ((!map.get(mapKeys.get(6)).toString().equals(""))
							|| (!map.get(mapKeys.get(6)).toString().isEmpty())) {
						String[] kycCategories = map.get(mapKeys.get(6)).toString().split("/");
					...
					String[] nonKYCFields = map.get(mapKeys.get(7)).toString().split("/");
					String[] mediumKYCFields = map.get(mapKeys.get(8)).toString().split("/");
					String[] fullKYCFields = map.get(mapKeys.get(9)).toString().split("/");
					String[] mandatoryFields = map.get(mapKeys.get(10)).toString().split("/");
					String[] showFields = map.get(mapKeys.get(11)).toString().split("/");
					String[] isEditableFields = map.get(mapKeys.get(12)).toString().split("/");
					String[] isEditableAdminFields = map.get(mapKeys.get(13)).toString().split("/");
				...
				if (verifyToolTip()) {
					...
					String nonDailyAmount[] = map.get(mapKeys.get(16)).toString().split(",");
					String nonWeeklyAmount[] = map.get(mapKeys.get(17)).toString().split(",");
					String nonMonthlyAmount[] = map.get(mapKeys.get(18)).toString().split(",");
					String mediumDailyAmount[] = map.get(mapKeys.get(21)).toString().split(",");
					String mediumWeeklyAmount[] = map.get(mapKeys.get(22)).toString().split(",");
					String mediumMonthlyAmount[] = map.get(mapKeys.get(23)).toString().split(",");
					String fullDailyAmount[] = map.get(mapKeys.get(26)).toString().split(",");
					String fullWeeklyAmount[] = map.get(mapKeys.get(27)).toString().split(",");
					String fullMonthlyAmount[] = map.get(mapKeys.get(28)).toString().split(",");

					if (verifyElement(nonKYCThreshold, false)) {
						sendTextInNonKYCMinimumAmount(map.get(mapKeys.get(14)).toString());
						sendTextInNonKYCMaximumAmount(map.get(mapKeys.get(15)).toString());
						sendTextInNonKYCDailyIn(nonDailyAmount[0]);
						sendTextInNonKYCDailyOut(nonDailyAmount[1]);
						sendTextInNonKYCDailyAll(nonDailyAmount[2]);
						sendTextInNonKYCWeeklyIn(nonWeeklyAmount[0]);
						sendTextInNonKYCWeeklyOut(nonWeeklyAmount[1]);
						sendTextInNonKYCWeeklyAll(nonWeeklyAmount[2]);
						sendTextInNonKYCMonthlyIn(nonMonthlyAmount[0]);
						sendTextInNonKYCMonthlyOut(nonMonthlyAmount[1]);
						sendTextInNonKYCMonthlyAll(nonMonthlyAmount[2]);
					}
					if (verifyElement(mediumKYCThreshold, false)) {
						sendTextInMediumKYCMinimumAmount(map.get(mapKeys.get(19)).toString());
						sendTextInMediumKYCMaximumAmount(map.get(mapKeys.get(20)).toString());
						sendTextInMediumKYCDailyIn(mediumDailyAmount[0]);
						sendTextInMediumKYCDailyOut(mediumDailyAmount[1]);
						sendTextInMediumKYCDailyAll(mediumDailyAmount[2]);
						sendTextInMediumKYCWeeklyIn(mediumWeeklyAmount[0]);
						sendTextInMediumKYCWeeklyOut(mediumWeeklyAmount[1]);
						sendTextInMediumKYCWeeklyAll(mediumWeeklyAmount[2]);
						sendTextInMediumKYCMonthlyIn(mediumMonthlyAmount[0]);
						sendTextInMediumKYCMonthlyOut(mediumMonthlyAmount[1]);
						sendTextInMediumKYCMonthlyAll(mediumMonthlyAmount[2]);
					}
					if (verifyElement(fullKYCThreshold, false)) {
						sendTextInFullKYCMinimumAmount(map.get(mapKeys.get(24)).toString());
						sendTextInFullKYCMaximumAmount(map.get(mapKeys.get(25)).toString());
						sendTextInFullKYCDailyIn(fullDailyAmount[0]);
						sendTextInFullKYCDailyOut(fullDailyAmount[1]);
						sendTextInFullKYCDailyAll(fullDailyAmount[2]);
						sendTextInFullKYCWeeklyIn(fullWeeklyAmount[0]);
						sendTextInFullKYCWeeklyOut(fullWeeklyAmount[1]);
						sendTextInFullKYCWeeklyAll(fullWeeklyAmount[2]);
						sendTextInFullKYCMonthlyIn(fullMonthlyAmount[0]);
						sendTextInFullKYCMonthlyOut(fullMonthlyAmount[1]);
						sendTextInFullKYCmonthlyAll(fullMonthlyAmount[2]);
					}
					clickOnNextBtn();
					if (verifyToolTip()) {
						clickOnCancelBtn();
						...
						String[] accessChannel = map.get(mapKeys.get(29)).toString().split(",");
						for (int i = 0; i < accessChannel.length; i++) {
							clickOnAccessChannel(accessChannel[i].trim());
						}
						clickOnNextBtn();
						sendTextInStakeHolderName(map.get(mapKeys.get(30)).toString());
						clickOnSaveBtn();
					}
				}
			}
	...
	public boolean verifyProfileAdded(Map<Object, Object> map, List<Object> mapKeys) {
		...
		By level = By
				.xpath("//label[text()[normalize-space() = 'Level']]//following-sibling :: div[normalize-space(text()) = '"
						+ map.get(mapKeys.get(3)).toString() + "']");
		By parentProfile = By
				.xpath("//label[text()[normalize-space() = 'Parent Profile']]//following-sibling :: div[normalize-space(text()) = '"
		...
		if (verifyElement(By.xpath("(//td[text()='" + map.get(mapKeys.get(0)).toString() + "'])[1]"), false)) {
			...
			if (map.get(mapKeys.get(1)).toString().contains("Non")) {
				...
				By nonMonthlyAll = By.xpath("//*[normalize-space(text())='" + ConstantsFile.MONTHLY_OVERALL_LIMIT
						+ "']//following-sibling::td[" + kycType + "]");
				if (!getUIText(nonMin).split(" ")[1].equals(map.get(mapKeys.get(14)).toString()))
					return false;
				if (!getUIText(nonMax).split(" ")[1].equals(map.get(mapKeys.get(15)).toString()))
					return false;
			...
			if (map.get(mapKeys.get(1)).toString().contains("Medium")) {
				...
				By mediumMonthlyAll = By.xpath("//*[normalize-space(text())='" + ConstantsFile.MONTHLY_OVERALL_LIMIT
						+ "']//following-sibling::td[" + kycType + "]");
				if (!getUIText(mediumMin).split(" ")[1].equals(map.get(mapKeys.get(19)).toString()))
					return false;
				if (!getUIText(mediumMax).split(" ")[1].equals(map.get(mapKeys.get(20)).toString()))
					return false;
			...
			if (map.get(mapKeys.get(1)).toString().contains("Full")) {
				...
				By fullMonthlyAll = By.xpath("//*[normalize-space(text())='" + ConstantsFile.MONTHLY_OVERALL_LIMIT
						+ "']//following-sibling::td[" + kycType + "]");
				if (!getUIText(fullMin).split(" ")[1].equals(map.get(mapKeys.get(24)).toString()))
					return false;
				if (!getUIText(fullMax).split(" ")[1].equals(map.get(mapKeys.get(25)).toString()))
					return false;
	...
	public void editProfile(Map<Object, Object> map, List<Object> mapKeys) {
		...
		if (verifyElement(By.xpath("(//td[text()='" + map.get(mapKeys.get(0)).toString() + "'])[1]"), false)) {
				...
				if (!map.get(mapKeys.get(6)).toString().equals("")) {
					...
					for (int m = 0; m < isEditableAdminFields.length; m++) {
						...
						if (!map.get(mapKeys.get(13)).toString().equals("")) {
							String[] isEditableAdmin = isEditableAdminField[1].split(",");
							if (verifyElement(
									By.xpath("//label[normalize-space(text())='" + isEditableAdminField[0] + "']"),
									false)) {
								for (int a = 0; a < isEditableAdmin.length; a++) {
									clickOnIsEditableAdminCheckBoxForEdit(isEditableAdmin[a]);
								}
							}
						}
					}
			...
			if (!map.get(mapKeys.get(29)).toString().equals("")) {
	...
	/**
	 * @author dishant.doshi to sort agent profile
	 * @param map
	 *            - for excel data
	 * @param mapKeys
	 *            - for excel header
